...
func TestSingleValidator(t *testing.T) {
	...
	str := simulation.GenerateBlockchainJsonString(weights, "", []interface{}{nil, nil, nil, nil, nil})
...
func TestZeroWeightValidator(t *testing.T) {
	weights := []uint64{5, 0}
	str := simulation.GenerateBlockchainJsonString(weights, "", []interface{}{nil, nil})
	p, err := NewBlockchainProtocol(str, 1)
	if err != nil {
		t.Errorf("error: %s", err)
		return
...
func TestZeroWeightBlock(t *testing.T) {
	...
	str := simulation.GenerateBlockchainJsonString(weights, "", []interface{}{nil, nil, nil, nil})
	p, err := NewBlockchainProtocol(str, 1)
	if err != nil {
		t.Errorf("error: %s", err)
		return
